...
public interface ProductoRepository extends JpaRepository<Producto, Integer> {
    ...
    public List<Producto> findByPrecioJPQL(@Param("precioInf") double precioInf, @Param("precioSup") double precioSup);
    
    //Ejemplo de método utilizando consultas SQL nativas
    @Query(nativeQuery = true,
            value = "SELECT * FROM producto p WHERE p.precio BETWEEN :precioInf AND :precioSup ORDER BY p.precio ASC")
    public List<Producto> findByPrecioSQL(@Param("precioInf") double precioInf, @Param("precioSup") double precioSup);
